...
interface Point { date: string; calls: number; cogs_inr: number; revenue_inr: number; new_users: number }
...
export default function TrendCharts() {
  ...
  const totalUsers = series.reduce((s, p) => s + (p.new_users || 0), 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-neutral-200 rounded-xl shadow-xs p-4">
          <div className="flex items-baseline justify-between mb-3">
            <p className="text-sm font-medium text-neutral-700">New users per day</p>
            <p className="text-xs text-neutral-400">{totalUsers.toLocaleString("en-IN")} total</p>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={series} margin={{ top: 4, right: 4, left: -18, bottom: 0 }}>
              <defs>
                <linearGradient id="users" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0ea5e9" stopOpacity={0.3} /><stop offset="100%" stopColor="#0ea5e9" stopOpacity={0} /></linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
              <XAxis dataKey="date" tickFormatter={label} tick={{ fontSize: 10, fill: "#94a3b8" }} interval="preserveStartEnd" />
              <YAxis tick={{ fontSize: 10, fill: "#94a3b8" }} allowDecimals={false} />
              <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
              <Area type="monotone" dataKey="new_users" name="New users" stroke="#0ea5e9" fill="url(#users)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
