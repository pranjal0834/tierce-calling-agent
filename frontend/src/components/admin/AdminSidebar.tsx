...
import {
  ...
  ShieldCheck, LogOut, ArrowLeft, ChevronLeft, ChevronRight, X,
  Hash, ArrowLeftRight, Bot, CalendarClock, Webhook,
  LayoutTemplate, MessageCircle, BookOpen,
} from "lucide-react";
  ...
  const content = (isMobile = false) => (
    <>
        ...
        {(isMobile || !collapsed) && (
          <div className="min-w-0 flex-1 animate-fade-in">
            ...
            <p className="text-[11px] text-brand-500 mt-1 leading-none flex items-center gap-1">
              <ShieldCheck className="icon-xs" /> Super Admin
            </p>
          </div>
        )}
        {isMobile && (
        ...
        {NAV.map(({ href, icon: Icon, label, badge }) => {
          ...
          return (
            ...
            >
              ...
              <Icon className={`icon-md flex-shrink-0 transition-colors ${active ? "text-brand-500" : "text-neutral-400 group-hover:text-neutral-600"}`} />
              {(isMobile || !collapsed) && <span className="truncate flex-1">{label}</span>}
              {/* Badge (expanded) */}
              {badge && kycPending > 0 && (isMobile || !collapsed) && (
                <span className="text-[10px] font-bold min-w-[18px] text-center px-1.5 py-0.5 rounded-full bg-error-500 text-white">{kycPending}</span>
              )}
              {/* Badge dot (collapsed) */}
              {badge && kycPending > 0 && !isMobile && collapsed && (
      ...
      <div className={`border-t border-neutral-100 pt-2 pb-3 space-y-0.5 ${!isMobile && collapsed ? "px-1.5" : "px-3"}`}>
        ...
        >
          <ArrowLeft className="icon-sm flex-shrink-0" />
        ...
        <button
          ...
          className={`group relative flex items-center gap-2.5 w-full rounded-lg text-[14px] font-medium text-neutral-400 hover:text-error-500 hover:bg-error-50 transition-colors duration-150 ${
            !isMobile && collapsed ? "justify-center h-9 w-9 mx-auto" : "px-2.5 py-2"
          }`}
        >
  ...
  return (
    <>
      ...
      <aside className={`relative hidden lg:flex flex-col bg-white border-r border-neutral-200 transition-all duration-250 ease-in-out flex-shrink-0 ${collapsed ? "w-[60px]" : "w-[260px]"}`}>
        ...
        >
          {collapsed ? <ChevronRight className="icon-xs text-neutral-500" /> : <ChevronLeft className="icon-xs text-neutral-500" />}
        </button>
      </aside>
    </>
  );
