    ...
    <>
      ...
      {loading || !stats ? <LoadingBlock /> : (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          <KpiStat label="Users"         value={stats.total_users}        icon={Users}       tint="bg-info-50 text-info-600"       />
          <KpiStat label="Total Calls"   value={stats.total_calls}        icon={Phone}       tint="bg-success-50 text-success-600" sub={`${stats.calls_last_24h} last 24h`} />
          <KpiStat label="Agents"        value={stats.total_agents}       icon={Bot}         tint="bg-purple-50 text-purple-600"   />
          <KpiStat label="Revenue (USD)" value={`$${stats.total_revenue_usd.toFixed(2)}`} icon={DollarSign} tint="bg-warning-50 text-warning-600" />
