...
import {
  Phone, Plus, Trash2, RefreshCw, Search, Bot,
...
import {
  ...
  updateNumberRouting, updateNumberAutoRenew, releasePhoneNumber, getAgents,
...
// Full country list for KYC submission (any country can be pre-verified)
      ...
      {local.provider === "twilio" ? (
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-xs text-gray-400 space-y-1">
              <p className="text-gray-300 font-medium">Using Plivo (India + global)</p>
              <p>Platform-managed account — no credentials needed. You can buy Indian numbers and 20+ other countries directly from this page.</p>
              <p>Numbers are billed via your Tierce credits at Plivo rates.</p>
            </div>
          </div>
        </div>
      ) : (
...
function NumberRow({ number, agents, onUpdate, onRelease }: {
  number: PhoneNumber;
  agents: Agent[];
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              ...
              {/* Monthly cost badge — shown in INR from number wallet */}
      ...
      <div className="mt-3 pt-3 border-t border-gray-800 flex items-center justify-between gap-3">
        ...
        <button
          ...
          className={`relative shrink-0 w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none ${
            autoRenew ? "bg-indigo-600" : "bg-gray-700"
          } ${togglingRenew ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
      ...
      <p className="text-xs text-gray-700 mt-3 font-mono">
        Purchased {new Date(number.purchased_at + "Z").toLocaleDateString("en-IN", {
...
function KycStatusBanner({ bundles, onOpenForm, onBundleUpdated }: {
  ...
  // Build display rows: required countries always shown + any extras already submitted
  ...
  // Countries not yet submitted — for the Add picker
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-800 shrink-0">
          <div>
            <h2 className="text-base font-semibold text-white">KYC — {countryName}</h2>
            <p className="text-xs text-gray-500 mt-0.5">
        ...
        <form onSubmit={submit} className="p-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="border-t border-gray-800 pt-4">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Authorized Signatory</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
...
function TrialBanner() {
  return (
    <div className="bg-gradient-to-r from-indigo-600/15 via-purple-600/10 to-indigo-600/15 border border-indigo-500/30 rounded-2xl p-5">
      <div className="flex items-start gap-4">
        <div className="w-10 h-10 bg-indigo-600/20 border border-indigo-500/30 rounded-xl flex items-center justify-center shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 leading-relaxed mb-3">
            Your AI agents can already make outbound calls using <span className="text-gray-200 font-medium">Tierce&apos;s shared platform number</span>.
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <a
              href="/billing"
              className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-xl transition-colors shrink-0"
            >
...
export default function PhoneNumbersPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (billing) {
        setWorkspacePlan(billing.plan ?? "free");
      }
    } catch {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Buy dedicated numbers for inbound calls and branded outbound caller ID</p>
          </div>
        </div>
        {isTrial ? (
          <a
      ...
      {/* Trial banner — only for free plan */}
      {isTrial && <TrialBanner />}

      {/* How it works — feature overview */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
          <div className="w-7 h-7 bg-blue-500/10 border border-blue-500/20 rounded-lg flex items-center justify-center mb-3">
            <Phone className="w-3.5 h-3.5 text-blue-400" />
          </div>
          <p className="text-sm font-semibold text-white mb-1">Inbound Calls</p>
          <p className="text-xs text-gray-500 leading-relaxed">
            Someone calls your number → Tierce instantly routes it to the assigned AI agent. No human needed.
          </p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
          <div className="w-7 h-7 bg-indigo-500/10 border border-indigo-500/20 rounded-lg flex items-center justify-center mb-3">
            <Bot className="w-3.5 h-3.5 text-indigo-400" />
          </div>
          <p className="text-sm font-semibold text-white mb-1">Outbound Caller ID</p>
          <p className="text-xs text-gray-500 leading-relaxed">
            When your agent calls out, your dedicated number appears as the caller ID instead of an unknown number.
          </p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4">
          <div className="w-7 h-7 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center justify-center mb-3">
            <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />
          </div>
          <p className="text-sm font-semibold text-white mb-1">Simple Billing</p>
          <p className="text-xs text-gray-500 leading-relaxed">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          <span className="w-5 h-5 bg-indigo-600 rounded-full text-[10px] font-bold text-white flex items-center justify-center shrink-0">1</span>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Choose your telephony provider</p>
        </div>
        <TelephonyProviderCard config={providerConfig} onChange={setProviderConfig} />
      </div>

      {/* KYC — required for regulated countries regardless of provider */}
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          <span className="w-5 h-5 bg-indigo-600 rounded-full text-[10px] font-bold text-white flex items-center justify-center shrink-0">2</span>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Buy numbers &amp; assign agents</p>
        </div>

        {/* Inbound routing explainer */}
        <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-4 mb-4">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">How inbound routing works</p>
          <div className="flex items-center gap-2 overflow-x-auto pb-1">
            {[
              { n: "1", label: "Caller dials your number", color: "text-blue-400 bg-blue-500/10 border-blue-500/20" },
              { n: "→", label: "", color: "" },
              { n: "2", label: "Tierce matches it to your agent", color: "text-indigo-400 bg-indigo-500/10 border-indigo-500/20" },
              { n: "→", label: "", color: "" },
              { n: "3", label: "AI agent answers in real-time", color: "text-green-400 bg-green-500/10 border-green-500/20" },
            ].map((item, i) =>
              item.n === "→" ? (
                <span key={i} className="text-gray-700 text-base shrink-0">→</span>
              ) : (
          ...
          <div className="mt-3 pt-3 border-t border-gray-800 flex items-start gap-2">
            ...
            <p className="text-xs text-gray-500">
              Each number can only route to <span className="text-gray-300">one agent</span>. You can change the routing at any time from the list below.
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-4 border border-dashed border-gray-700/60 rounded-2xl">
          ...
          <div className="text-center">
            <p className="text-sm font-medium text-gray-400">No phone numbers yet</p>
